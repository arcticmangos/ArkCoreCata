...
#endif
Unit::Unit () :
        WorldObject(), m_movedPlayer(NULL), IsAIEnabled(false), NeedChangeAI(false), m_ControlledByPlayer(false), i_AI(NULL), i_disabledAI(NULL), m_procDeep(0), m_removedAurasCount(0), i_motionMaster(this), m_ThreatManager(this), m_vehicle(NULL), m_vehicleKit(NULL), m_unitTypeMask(UNIT_MASK_NONE), m_HostileRefManager(this), m_lastSanctuaryTime(0)
{
#ifdef _MSC_VER
...
void Unit::AttackerStateUpdate (Unit *pVictim, WeaponAttackType attType, bool extra)
{
    ...
    else
    {
        ...
        if (GetTypeId() == TYPEID_PLAYER)
            sLog->outStaticDebug("AttackerStateUpdate: (Player) %u attacked %u (TypeId: %u) for %u dmg, absorbed %u, blocked %u, resisted %u.", GetGUIDLow(), pVictim->GetGUIDLow(), pVictim->GetTypeId(), damageInfo.damage, damageInfo.absorb, damageInfo.blocked_amount, damageInfo.resist);
        else
            sLog->outStaticDebug("AttackerStateUpdate: (NPC)    %u attacked %u (TypeId: %u) for %u dmg, absorbed %u, blocked %u, resisted %u.", GetGUIDLow(), pVictim->GetGUIDLow(), pVictim->GetTypeId(), damageInfo.damage, damageInfo.absorb, damageInfo.blocked_amount, damageInfo.resist);
    }
...
MeleeHitOutcome Unit::RollMeleeOutcomeAgainst (const Unit *pVictim, WeaponAttackType attType, int32 crit_chance, int32 miss_chance, int32 dodge_chance, int32 parry_chance, int32 block_chance) const
{
    ...
    sLog->outStaticDebug("RollMeleeOutcomeAgainst: rolled %d, miss %d, dodge %d, parry %d, block %d, crit %d", roll, miss_chance, dodge_chance, parry_chance, block_chance, crit_chance);
    ...
    if (attType != RANGED_ATTACK && (GetTypeId() == TYPEID_PLAYER || this->ToCreature()->isPet()) && pVictim->GetTypeId() != TYPEID_PLAYER && !pVictim->ToCreature()->isPet() && getLevel() < pVictim->getLevelForTarget(this))
    {
    ...
    !IsControlledByPlayer() && !(GetTypeId() == TYPEID_UNIT && this->ToCreature()->GetCreatureInfo()->flags_extra & CREATURE_FLAG_EXTRA_NO_CRUSH))
    {
...
bool Unit::isSpellBlocked (Unit *pVictim, SpellEntry const * spellProto, WeaponAttackType attackType)
{
    if (pVictim->HasInArc(M_PI, this) || pVictim->HasAuraType(SPELL_AURA_IGNORE_HIT_DIRECTION))
    {
        ...
        if (pVictim->GetTypeId() == TYPEID_UNIT && pVictim->ToCreature()->GetCreatureInfo()->flags_extra & CREATURE_FLAG_EXTRA_NO_BLOCK)
            return false;
...
uint32 Unit::GetDefenseSkillValue (Unit const* target) const
{
    if (GetTypeId() == TYPEID_PLAYER)
    {
        ...
        uint32 value = (target && target->GetTypeId() == TYPEID_PLAYER) ? this->ToPlayer()->GetMaxSkillValue(SKILL_DEFENSE) : this->ToPlayer()->GetSkillValue(SKILL_DEFENSE);
...
uint32 Unit::GetWeaponSkillValue (WeaponAttackType attType, Unit const* target) const
{
    ...
    if (GetTypeId() == TYPEID_PLAYER)
    {
        ...
        // in PvP use full skill instead current skill value
        value = (target && target->IsControlledByPlayer()) ? this->ToPlayer()->GetMaxSkillValue(skill) : this->ToPlayer()->GetSkillValue(skill);
        // Modify value from ratings
        value += uint32(this->ToPlayer()->GetRatingBonusValue(CR_WEAPON_SKILL));
        switch (attType)
        {
        case BASE_ATTACK:
            value += uint32(this->ToPlayer()->GetRatingBonusValue(CR_WEAPON_SKILL_MAINHAND));
            break;
        case OFF_ATTACK:
            value += uint32(this->ToPlayer()->GetRatingBonusValue(CR_WEAPON_SKILL_OFFHAND));
            break;
        case RANGED_ATTACK:
            value += uint32(this->ToPlayer()->GetRatingBonusValue(CR_WEAPON_SKILL_RANGED));
            break;
        default:
            break;
        }
...
void Unit::_UpdateAutoRepeatSpell ()
{
    ...
    if (IsNonMeleeSpellCasted(false, false, true, m_currentSpells[CURRENT_AUTOREPEAT_SPELL]->m_spellInfo->Id == 75))
    {
        // cancel wand shoot
...
bool Unit::CanCastWhileWalking (const SpellEntry * const sp)
{
    ...
    for (AuraEffectList::const_iterator i = alist.begin(); i != alist.end(); ++i)
    {
        ...
        if (!((*i)->GetSpellProto()->EffectSpellClassMask[(*i)->GetEffIndex()] & sp->SpellFamilyFlags))
            continue;
        return true;
    }
    return false;
...
AuraApplication * Unit::_CreateAuraApplication (Aura * aura, uint8 effMask)
{
    ...
    if (!isAlive() && !IsDeathPersistentSpell(aurSpellInfo) && (GetTypeId() != TYPEID_PLAYER || !this->ToPlayer()->GetSession()->PlayerLoading()))
        return NULL;
...
void Unit::RemoveAuraFromStack (uint32 spellId, uint64 caster, AuraRemoveMode removeMode)
{
    for (AuraMap::iterator iter = m_ownedAuras.lower_bound(spellId); iter != m_ownedAuras.upper_bound(spellId);)
    {
        ...
        if ((aura->GetType() == UNIT_AURA_TYPE) && (!caster || aura->GetCasterGUID() == caster))
        {
...
{
    static const AuraType diseaseAuraTypes[] =
    { SPELL_AURA_PERIODIC_DAMAGE, SPELL_AURA_PERIODIC_DAMAGE_PERCENT, SPELL_AURA_NONE };
...
bool Unit::HandleDummyAuraProc (Unit *pVictim, uint32 damage, AuraEffect* triggeredByAura, SpellEntry const * procSpell, uint32 procFlag, uint32 procEx, uint32 cooldown)
{
    ...
    case SPELLFAMILY_MAGE:
    {
        ...
        case 56372:
        {
            ...
            for (SpellCooldowns::const_iterator itr = cooldowns.begin(); itr != cooldowns.end(); ++itr)
            {
                ...
                // Frost Nova
                if (cdSpell && cdSpell->SpellFamilyName == SPELLFAMILY_MAGE && cdSpell->SpellFamilyFlags[0] & 0x00000040)
    ...
    case SPELLFAMILY_PALADIN:
    {
        ...
        if (dummySpell->Id == 20154)
        {
            ...
            int32 holy = SpellBaseDamageBonus(SPELL_SCHOOL_MASK_HOLY) + SpellBaseDamageBonusForVictim(SPELL_SCHOOL_MASK_HOLY, pVictim);
    ...
    case SPELLFAMILY_SHAMAN:
    {
        ...
        if (dummySpell->SpellFamilyFlags[0] & 0x200000)
        {
            ...
            float add_spellpower = (float) (SpellBaseDamageBonus(SPELL_SCHOOL_MASK_FIRE) + SpellBaseDamageBonusForVictim(SPELL_SCHOOL_MASK_FIRE, pVictim));
    ...
    case SPELLFAMILY_DEATHKNIGHT:
    {
        ...
        if (dummySpell->SpellIconID == 1939 && GetTypeId() == TYPEID_PLAYER)
        {
            ...
            for (PlayerSpellMap::const_iterator itr = sp_list.begin(); itr != sp_list.end(); ++itr)
            {
                ...
                if (spellProto->SpellFamilyName == SPELLFAMILY_DEATHKNIGHT && spellProto->SpellFamilyFlags[0] & 0x2000)
                {
            ...
            *handled = true;
            ...
            if (GetTypeId() == TYPEID_PLAYER)
            {
                ...
                for (uint8 i = 0; i < MAX_RUNES && runesLeft; ++i)
                {
                    if (dummySpell->SpellIconID == 2622)
                    {
                        if (((Player*) this)->GetCurrentRune(i) == RUNE_DEATH || ((Player*) this)->GetBaseRune(i) == RUNE_BLOOD)
                            continue;
                    }
                    else
                    {
...
bool Unit::HandleProcTriggerSpell (Unit *pVictim, uint32 damage, AuraEffect* triggeredByAura, SpellEntry const *procSpell, uint32 procFlags, uint32 procEx, uint32 cooldown)
{
    ...
    {
        // Proc off Aimed Shot
        if (!(procSpell->SpellFamilyFlags[0] & 0x00020000))
            return false;
        break;
    }
...
uint32 Unit::SpellDamageBonus (Unit *pVictim, SpellEntry const *spellProto, uint32 effIndex, uint32 pdamage, DamageEffectType damagetype, uint32 stack)
{
    ...
    for (AuraEffectList::const_iterator i = mModDamagePercentDone.begin(); i != mModDamagePercentDone.end(); ++i)
        if (((*i)->GetMiscValue() & GetSpellSchoolMask(spellProto)) && (*i)->GetSpellProto()->EquippedItemClass == -1 &&          // -1 == any item class (not wand)
...
int32 Unit::SpellBaseDamageBonus (SpellSchoolMask schoolMask)
{
    ...
    for (AuraEffectList::const_iterator i = mDamageDone.begin(); i != mDamageDone.end(); ++i)
        if (((*i)->GetMiscValue() & schoolMask) != 0 && (*i)->GetSpellProto()->EquippedItemClass == -1 &&
...
uint32 Unit::SpellHealingBonus (Unit *pVictim, SpellEntry const *spellProto, uint32 effIndex, uint32 healamount, DamageEffectType damagetype, uint32 stack)
{
    ...
    if (bonus)
    {
        if (damagetype == DOT)
        {
            coeff = bonus->dot_damage;
            if (bonus->ap_dot_bonus > 0)
                DoneTotal += int32(bonus->ap_dot_bonus * stack * GetTotalAttackPowerValue((IsRangedWeaponSpell(spellProto) && spellProto->DmgClass != SPELL_DAMAGE_CLASS_MELEE) ? RANGED_ATTACK : BASE_ATTACK));
        }
        else
        {
            coeff = bonus->direct_damage;
            if (bonus->ap_bonus > 0)
                DoneTotal += int32(bonus->ap_bonus * stack * GetTotalAttackPowerValue((IsRangedWeaponSpell(spellProto) && spellProto->DmgClass != SPELL_DAMAGE_CLASS_MELEE) ? RANGED_ATTACK : BASE_ATTACK));
        }
    }
    else          // scripted bonus
    {
...
bool Unit::IsImmunedToSpell (SpellEntry const* spellInfo)
{
    ...
    if (spellInfo->Id != 42292 && spellInfo->Id != 59752)
    {
        SpellImmuneList const& schoolList = m_spellImmune[IMMUNITY_SCHOOL];
        for (SpellImmuneList::const_iterator itr = schoolList.begin(); itr != schoolList.end(); ++itr)
            if ((itr->type & GetSpellSchoolMask(spellInfo)) && !(IsPositiveSpell(itr->spellId) && IsPositiveSpell(spellInfo->Id)) && !CanSpellPierceImmuneAura(spellInfo, sSpellStore.LookupEntry(itr->spellId)))
                return true;
    }
...
bool Unit::IsImmunedToSpellEffect (SpellEntry const* spellInfo, uint32 index) const
{
    ...
    if (uint32 aura = spellInfo->EffectApplyAuraName[index])
    {
        ...
        for (AuraEffectList::const_iterator iter = immuneAuraApply.begin(); iter != immuneAuraApply.end(); ++iter)
            ...
            spellInfo->Dispel == DISPEL_DISEASE || spellInfo->Dispel == DISPEL_POISON) && ((*iter)->GetMiscValue() & GetSpellSchoolMask(spellInfo)) &&          // Check school
...
bool Unit::canAttack (Unit const* target, bool force) const
{
    ...
    if (target->HasUnitState(UNIT_STAT_UNATTACKABLE) || (target->GetTypeId() == TYPEID_PLAYER && target->ToPlayer()->isGameMaster()))
        return false;

    // check flags
    if (target->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_TAXI_FLIGHT | UNIT_FLAG_NOT_ATTACKABLE_1 | UNIT_FLAG_UNK_16) || (!HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP_ATTACKABLE) && target->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PASSIVE)) || (!target->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP_ATTACKABLE) && HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PASSIVE)) || (HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP_ATTACKABLE) && target->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_OOC_NOT_ATTACKABLE)) || (target->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP_ATTACKABLE) && HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_OOC_NOT_ATTACKABLE)))
        return false;
...
float Unit::ApplyDiminishingToDuration (DiminishingGroup group, int32 &duration, Unit *caster, DiminishingLevels Level, int32 limitduration)
{
    ...
    if (limitduration > 0 && duration > limitduration)
    {
        ...
        if ((target->GetTypeId() == TYPEID_PLAYER || ((Creature*) target)->GetCreatureInfo()->flags_extra & CREATURE_FLAG_EXTRA_ALL_DIMINISH) && source->GetTypeId() == TYPEID_PLAYER)
    ...
    else if ((GetDiminishingReturnsGroupType(group) == DRTYPE_PLAYER && ((targetOwner ? (targetOwner->GetTypeId() == TYPEID_PLAYER) : (GetTypeId() == TYPEID_PLAYER)) || (GetTypeId() == TYPEID_UNIT && ((Creature*) this)->GetCreatureInfo()->flags_extra & CREATURE_FLAG_EXTRA_ALL_DIMINISH))) || GetDiminishingReturnsGroupType(group) == DRTYPE_ALL)
    {
        DiminishingLevels diminish = Level;
        switch (diminish)
        {
...
CharmInfo::CharmInfo (Unit* unit) :
        m_unit(unit), m_CommandState(COMMAND_FOLLOW), m_petnumber(0), m_barInit(false), m_isCommandAttack(false), m_isAtStay(false), m_isFollowing(false), m_isReturning(false)
{
    ...
    for (ProcTriggeredList::const_iterator i = procTriggered.begin(); i != procTriggered.end(); ++i)
    {
        ...
        if (!handled)
            for (uint8 effIndex = 0; effIndex < MAX_SPELL_EFFECTS; ++effIndex)
            {
                ...
                {
                    sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "ProcDamageAndSpell: casting mending (triggered by %s dummy aura of spell %u)", (isVictim ? "a victim's" : "an attacker's"), triggeredByAura->GetId());
